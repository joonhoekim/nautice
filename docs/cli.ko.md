...
에이전트의 훅과 지침은 옵션 이름으로 nautice 를 부르므로, 옵션 이름이 바뀌면
조용히 깨진다. **이 계약이 공개 API 다:** 명령, 옵션과 그 단위·범위, 환경변수,
효과음 이름표, 종료 코드. 여기에 들지 않는 것: `--plan` 출력(테스트용), 상태 줄·
에러·`doctor` 의 문구, 캐시 배치.

- **옵션이나 환경변수를 지우거나 이름을 바꿀 때**: 옛 이름은 적어도 한 번의 minor
  릴리스 동안 계속 동작하면서 stderr 로 폐기 경고를 찍고, 그 뒤에 없앤다.
- **버전**: 0.x 동안은 깨지는 변경이면 minor, 나머지는 patch 를 올린다. 1.0 부터는
  semantic versioning 을 따른다.
- **기본값 변경**(`call` 의 문구 같은)은 깨지는 변경으로 보지 않지만 릴리스 노트에
  따로 적는다.
