...
- **에이전트 설정을 건드리지 않는다.** 훅과 지침 파일은 에이전트마다 이름이
  다르고, 사용자마다 원하는 방식도 다르다. 인스톨러가 쓴 설정은 낡아도 아무도
  고치지 않는다. `README.md` 가 에이전트에게 줄 프롬프트를 주고, 붙이는 것은
  사람이 한다.
...
설치 한 줄을 다시 돌리면 제자리에서 업데이트된다. `nautice update` 는 자기가 깔린
prefix 에 대해 바로 그 일을 한다(`docs/cli.ko.md`). 업데이트 로직이 곧 설치 로직이다.
...
설치 경로를 세 OS 에서 끝까지 돌린다 — `ci.yml` 의 `install` 잡이다.
